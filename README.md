...
- 安装包：`src-tauri\target\release\bundle\nsis\cloudflare-manager_0.2.0_x64-setup.exe`
...
│   ├── gen/                      # Tauri 构建生成的 schema（无需手动修改）
│   ├── icons/                    # 应用图标（多尺寸 PNG / ICO / ICNS）
│   ├── src/
│   │   ├── main.rs               # 可执行入口
│   │   └── lib.rs                # Tauri 命令与 Cloudflare API 代理
│   ├── .gitignore
│   ├── Cargo.lock
│   ├── Cargo.toml
│   ├── build.rs
...
# cloudflare-manager-v0.2.0-src.zip   # 源码分发包（由 git-push.bat 选项 7 生成）
```
...
- 安装包：`cloudflare-manager_0.2.0_x64-setup.exe`
- 可执行文件：`cloudflare-manager.exe`
- 源码包：`cloudflare-manager-v0.2.0-src.zip`（由 `git-push.bat` 选项 7 或手动打包生成）

> 以上分发文件均已被 `.gitignore` 排除，不要直接提交到 Git 仓库。安装包和可执行文件建议上传到 GitHub Release 附件。
